          ...
          <div class="row mb-3">
            ...
            <div class="col-sm-9">
              <select class="form-select form-select-sm" id="status" name="status" required>
                <?php 
              // Ambil status
              $status_options = getEnum('status', 'pesanan_pembelian');
...
$(document).ready(function() {
  ...
  $(document).on('click', '.remove-btn', function() {
    var idDetailPesanan = $(this).closest('.main-tr').find('input[name="id_detail_pesanan[]"]').val();
    ...
    // Tambahkan idDetailPesanan ke #deleted-rows dan cetak konten #deleted-rows ke konsol
    $('#deleted-rows').append('<input type="hidden" name="deleted_rows[]" value="' + idDetailPesanan + '">');
  });
  ...
  $(document).on('input', '.qty, .price', function() {
    ...
    var idDetailPesanan = row.find('input[name="id_detail_pesanan[]"]').val(); // Dapatkan ID detail
